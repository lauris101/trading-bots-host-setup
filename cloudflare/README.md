...
The Cloudflare side of the two hosts, as Terraform: one tunnel per host, a
...
The services host (Uptime Kuma, Grafana; its own DNS, outside this
configuration) is only on the bypass list: its addresses in `bypass_cidrs`
let its monitors reach the hostnames without a login.
