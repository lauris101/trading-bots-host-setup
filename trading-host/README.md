...
The AWS trading host, as code (one of the three parts of trading-bots-host-setup; the
repository README explains the order). Two layers, two tools, run from your laptop:
...
| the OS | **ansible** (`playbook.yml`, roles in `../ansible/roles`) | packages, UTC + Amazon time sync, sshd hardening, the `trading-bot` account with the SSH key you choose (sudo, docker group), Docker Engine + compose, CrowdSec with the nftables bouncer, a unit that puts the secondary IPs on the interface, kernel core isolation and network sysctls, `/data` and `/logs` roots |
...
`inventory/hosts.yml` from the terraform outputs (first elastic IP,
...
```bash
cp vars.yml.example vars.yml     # the trading-bot key, cores, dirs
